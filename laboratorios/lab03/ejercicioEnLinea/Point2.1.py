...
class BacktrackingMinCost:
    ...
    def minumumCostAux(self, g):
        valued = [False]*g.size
        cost = self.minimumCost(g, 0, g.size-1, math.inf, 0, valued, "0")
        if cost == math.inf:
    ...
    def minimumCost(self, g, v, w, minCost, accumulated, valued, path):
        valued[v] = True
        if v == w:
            ...
            for son in children:
                ...
                if (fatherSonCost < minCost) and (v != son) and valued[son] is False:
                    minCost = \
                        min(self.minimumCost
                            (g, son, w, minCost, accumulated + g.getWeight(v, son), valued, path + str(son))[0], minCost)
                    valued[son] = False
            return [minCost, path]
